...
import { toast } from "sonner";
import { useDispatch, useSelector } from "react-redux";
import { loginUser, googleAuth } from "@/redux/services/userAuthServices";
import ReactLoading from "react-loading";
import GoogleLoginButton from "../../componets/GoogleLoginButton";
import { signInWithPopup } from "firebase/auth";
import { auth, provider } from "@/utils/fireBase/config";
        ...
        >
          {({ handleChange, values, touched, errors, isSubmitting }) => (
            <Form className="space-y-4">
              ...
              {/* Email */}
              <div>
                <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-gray-300 focus-within:border-indigo-500 transition">
              ...
              <button
                type="submit"
                disabled={isSubmitting}
                className="w-full flex justify-center items-center gap-2 bg-gradient-to-r from-indigo-600 to-purple-700 text-white py-3 rounded-xl font-semibold hover:opacity-95 transition"
              >
                {isSubmitting ? (
