...
const MenteeHome = () => {
  ...
  {mentors.map((mentor, index) => (
    <div  key={index} className="bg-white p-6 rounded-lg shadow-sm flex flex-col items-center">
      ...
      <h2 className="text-lg font-bold font-inter text-center mb-1">{mentor.name}</h2>
      <div className="flex items-center mb-4">
        <img src={`https://flagcdn.com/${mentor.location.toLowerCase()}.svg`} alt={mentor.location} className="w-6 h-6 mr-2" />
        <p className="text-gray-500">{mentor.location}</p>
      </div>
      <p className="text-gray-500 mb-4 text-center">{mentor.jobTitle}</p>
      <p className="text-gray-700 mb-4 text-center">
        {mentor.skills.join(', ')}
      </p>
      <button onClick={()=>handleCardClick(mentor._id)} className="px-4 py-2 bg-white text-indigo-500 border border-indigo-500 rounded-full shadow-md transition duration-300 ease-in-out hover:bg-indigo-500 hover:text-white">
        explore
      </button>
    </div>
  ))}
